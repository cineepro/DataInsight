...
const COLLECTION_ID = import.meta.env.VITE_COLLECTION_SCANS_PHARMACIE;

type ScanPharmacieInput = Omit<ScanPharmacie, 'timestamp' | 'year' | 'week_number' | 'day_of_week'>;

export async function submitPharmacieScan(input: ScanPharmacieInput): Promise<void> {
  const now = new Date();
  const { year, week_number, day_of_week } = getISOYearWeek(now);

  const payload: ScanPharmacie = {
    ...input,
