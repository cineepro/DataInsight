...
const FUNCTION_ID = 'common.churn_risk';
...
export async function detectChurnRisk(customers: Customer[], now: Date = new Date()): Promise<ChurnAnalysisEntry[]> {
  ...
  return customers.map((customer) => {
    ...
    if (averageIntervalDays !== null) {
      if (daysSinceLastVisit >= averageIntervalDays * t.churned_multiplier) {
        riskLevel = 'CHURNED';
      } else if (daysSinceLastVisit >= averageIntervalDays * t.at_risk_multiplier) {
        riskLevel = 'AT_RISK';
      }
