//apps/studio/src/engine/flexible/aggregateByDimension.ts
import type { DatasetRow, DatasetColumnDef, AggregationType, AnalysisResult } from './types';
...
/**
 * Équivalent d'un tableau croisé dynamique simple : regroupe les lignes
 * par la valeur d'une colonne "dimension" (ex: Ville, Produit), et calcule
 * une somme/moyenne/compte sur une colonne "mesure" (ex: Ventes).
 */
export function aggregateByDimension(
  ...
  const results: AggregateGroup[] = [...groups.entries()]
    .map(([dimensionValue, values]) => {
      ...
      else value = values.length; // COUNT
